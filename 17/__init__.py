...
class ProbeTargetingSystem(object):
    ...
    def over_shot(self):
        if self.track[-1][0] > self.target['x2']:
            return True
    ...
    def fire(self, x_vel, y_vel):
        ...
        self.x_pos = 0
        self.y_pos = 0
        self.x_vel_init = self.x_vel = x_vel
        self.y_vel_init = self.y_vel = y_vel

        self.add_track_point(self.x_pos, self.y_pos)

        while self.x_pos < self.target['x2'] and self.y_pos > self.target['y2']:
            self.step()
...
def find_coolest_shot(pts):
    ...
    for i in range(0, 1000):
        ...
        print(i, 'HIT' if hit else 'MISS', highest, 'xv', this_xv, 'yv', this_yv, hit, 'over shot' if over_shot else '')
